...
class Diet : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val bottomNavigationView =
            findViewById<BottomNavigationView>(R.id.bottomNavigationDiet)
    ...
    private fun loadFoods()
    {
        ...
        foodsList.setUp("foods", "diet", prefs.getCurrentDate(), Color.BLACK)
    ...
    private fun writeDictionary()
    {
        ...
        val write = if (writeList.isEmpty()) { "ERROR: DATA NOT FOUND" }
            else { writeList.joinToString(";") }
        prefs.writeSetting("FOODDICTIONARY", write)
    }

    //when the + (add food to dictionary) button is clicked,
    // this is called to handle the dialog box
    private fun addFoodWindow()
    {
        ...
        val builder = AlertDialog.Builder(this)
            .setView(dialogView)
            .setTitle("Add a new food to the dictionary")
            ...
            .setPositiveButton("Add",
                ...
                DialogInterface.OnClickListener { _, _ ->
                    ...
                    val calories =
                        dialogView.foodCaloriesInput.foodCaloriesInputText.text.toString()
                    var tags =
                        dialogView.foodTagsInput.foodTagsInputText.text.toString()
                    val validEntries = validateDialogEntry(name, calories, tags)
                        ?: return@OnClickListener
                    //if there are no errors,
                    // the item is added to the live list and the dictionary
                    val caloriesInt = validEntries.first
                    tags = validEntries.second
                    foodsList.addItem("$name#$caloriesInt#$tags")
                    foodDictionary[name] = validEntries
                    foodDictionary = foodDictionary.toSortedMap().toMutableMap()
                    writeDictionary()
                    updateDictionaryView(null)
                })
    ...
    //checks that the user's entered values in the add food dialog box are valid
    //returns null if not valid
    //returns the calories as int and tags as string in pair if valid
    private fun validateDialogEntry(
        name: String, calories: String, tags: String) : Pair<Int, String>?
    {
        //checks that the name entered is alphanumeric and not empty
        if (name == "" || name == " " || !(name.matches("^[a-zA-Z0-9 ]*$".toRegex())))
        {
            Toast.makeText(this,
                "Food name should be alphanumeric", Toast.LENGTH_SHORT).show()
            return null
        }
        //checks that the name hasn't already been added
        if (name in foodDictionary.keys)
        {
            Toast.makeText(this,
                "You have already added this food to your dictionary",
                Toast.LENGTH_SHORT).show()
            return null
        }
        //checks that the calories is a natural number
        ...
        if (calories != "") {
            try {
                ...
                Toast.makeText(
                    this,
                    "You can leave the calories box blank, or it must be a whole number",
                    Toast.LENGTH_SHORT
                ).show()
                return null
            }
        ...
        if (tags != "")
        {
            ...
            {
                Toast.makeText(this,
                    "You can leave the tags box blank, or list them separated by commas",
                    Toast.LENGTH_SHORT).show()
                return null
            }
        ...
        return Pair(caloriesInt, tagString)
    }
        ...
        else
        {
            for (food in foods)
            {
                ...
                foodView.deleteFoodButton.setOnClickListener() {
                    ...
                    //the item is removed from the dictionary and any
                    // instances of that item in the live list are removed
