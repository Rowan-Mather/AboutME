...
//the compound view for the calendar date activity containing
// a text view and +/- buttons
class EditButtons : ConstraintLayout {
    ...
    fun setUp(
        button: EditButtonAttributes, colour: Int, thisDate: String, thisCategory: String)
    {
        if (button.isIntRange())
        {
            setRange(button.minimum, button.maximum)
            if (button.minimum - button.maximum == 0)
            {
                editAdd.hide()
                editDelete.hide()
            }
    ...
    private fun editButtonClicked(change: Int)
    {
        ...
        {
            val ranged = range!!
            //if the attribute doesn't have an assigned value yet,
            // it is set to the first or last item in the range depending
            // on whether it was the + or - button clicked
            if (selected == null)
            {
                selected = if (change > 0) { ranged[0] }
                else { ranged[ranged.lastIndex] }
            }
            //otherwise the current selected item is indexed and changed
            // (if the change is allowed for the range)
            else
            {
                val newIndex: Int = ranged.indexOf(selected) + change
                if (newIndex < 0 || newIndex > ranged.lastIndex)
                {
                    Toast.makeText(context,"Outside the range",
