...
//a compound view for the calendar date activity
// containing a text view and then a customisable list of items
class ListEditButton : ConstraintLayout {
    ...
    //this is a standard snippet for this type of class that I copied
    @JvmOverloads
    constructor(
        context: Context?,
    ...
    //allows the user to set the name of the view and colour of the + button &
    //calls loadList also if the view is the diet food list and has calories
    // and tags, these are added
            ...
            val builder = AlertDialog.Builder(context)
                ...
                .setPositiveButton("Add",
                    DialogInterface.OnClickListener { _, _ ->
                        val name =
                            dialogView.foodNameInput.foodNameInputText.text.toString()
                        val calories =
                            dialogView.foodCaloriesInput.foodCaloriesInputText
                                .text.toString()
                        val tags =
                            dialogView.foodTagsInput.foodTagsInputText.text.toString()
                        //checks that the calories entered is valid,
                        // if left empty, value defaults to -1
                        var caloriesInt = -1
                        try {
                        ...
                        catch (e: Exception)
                        {
                            if (calories != "") {
                                Toast.makeText(context,
                                    "You can leave the calories box blank, or it must" +
                                            " be a whole number",
                                    Toast.LENGTH_SHORT).show()
                                return@OnClickListener
                            }
                        }
                        //edits the tags entered so they are saved in the form
                        // 'tag1!tag2!tag3'
                        var tagString = ""
                        if (tags != "")
                        {
                            ...
                            {
                                Toast.makeText(context,
                                    "You can leave the tags box blank, or list " +
                        ...
                        //refresh page
                        val dateIntent = Intent(context, CalendarDate::class.java)
                        dateIntent.putExtra("selectedDate", thisDate)
                        context.startActivity(dateIntent)
                        (context as Activity).overridePendingTransition(0,0)
                    })
            builder.show()
        }
        else if (listName == "medication")
        {
            val dialogView = LayoutInflater.from(context).inflate(
                R.layout.add_medication_dialog, null)
            //a builder for the window is initialised with a title
            val builder = AlertDialog.Builder(context)
                .setView(dialogView)
                ...
                .setPositiveButton("Add",
                    ...
                    DialogInterface.OnClickListener { _, _ ->
                        val name =
                            dialogView.medicationName.medicationNameInput.text.toString()
                        val dose =
                            dialogView.medicationDose.medicationDoseInput.text.toString()
                        val frequency =
                            dialogView.medicationFrequency.medicationFrequencyInput
                                .text.toString()
                        if (name == "" || name == " " ||
                            !(name.matches("^[a-zA-Z0-9 ]*$".toRegex())))
                        {
                            Toast.makeText(context,
                        ...
                        if (dose != "")
                        {
                            ...
                            {
                                Toast.makeText(context,
                                    "You can leave the dose blank, or " +
                        ...
                        {
                            if (frequency == " " ||
                                !(frequency.matches("^[a-zA-Z0-9 ]*$".toRegex())))
                            {
                                Toast.makeText(context,
                                    "You can leave the frequency blank, " +
                                            "or it must be alphanumeric",
                                    Toast.LENGTH_SHORT).show()
                                return@OnClickListener
                            }
                        }
                        addItem("$name#$dose#${frequency.toLowerCase()}")
                    })
            builder.show()
        }
        else {
            ...
            val dialogView = LayoutInflater.from(context).inflate(
                R.layout.add_list_item_dialog, null)
            //a builder for the window is initialised with a title
            val builder = AlertDialog.Builder(context)
                .setView(dialogView)
                ...
                .setPositiveButton("Add"
                    ...
                    //the value is checked to not already be in the list
                    // and that its alphanumeric, then added to the list
    ...
    fun addItem(item: String)
    {
        ...
        editList.add(item)
        writeList()
        //the item is stored as a text view and a delete floating action button
        // which are nested in a horizontal linear layout
        ...
        if (listName == "medication"){
            val itemSplit = item.split("#")
            try
            { symptomNameView.text = "${itemSplit[0]} (${itemSplit[1]}mcg)"
            } catch (e: Exception) {}
        }
    ...
    private fun configureDeleteButton(deleteButton: FloatingActionButton, index: Int)
    {
        ...
        deleteButton.backgroundTintList =
            ColorStateList.valueOf(Color.parseColor("#FF8080"))
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            deleteButton.imageTintList =
    ...
    private fun deleteItem(index: Int)
    {
        ...
        writeList()
        if (listName == "foods") { calculateTotalCalories() }
    }
    ...
    {
        prefs.writeData(
            thisDate, listCategory, listName, editList.joinToString("+"))
    }
    ...
    private fun calculateTotalCalories()
    {
        ...
        prefs.writeData(
            thisDate, "diet", "total calories", total.toString())
    }
}